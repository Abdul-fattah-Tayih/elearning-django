...
from dashboard.forms import AddCourseForm, EditCourseForm, LessonForm
...
def lesson_detail(request: HttpRequest, course_id: int, lesson_id: int):
    course = Course.objects.filter(participants__id=request.user.id, id=course_id).prefetch_related('lesson_set').get()
    lesson = Lesson.objects.filter(id=lesson_id, course__id=course_id, course__participants__id=request.user.id).get()
...
class AddLessonView(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def get(self, request: HttpRequest, course_id):
        course = Course.objects.filter(participants__id=request.user.id, id=course_id).get()
    ...
    def post(self, request: HttpRequest, course_id: int):
        ...
        if form.is_valid():
            Lesson.objects.create(name=form.cleaned_data.get('name'), content=form.cleaned_data.get('content'), course=course, created_by=request.user)
...
class EditLessonView(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def get(self, request: HttpRequest, course_id: int, lesson_id: int):
        lesson = lesson = Lesson.objects.filter(id=lesson_id, course__id=course_id, course__participants__id=request.user.id).get()
